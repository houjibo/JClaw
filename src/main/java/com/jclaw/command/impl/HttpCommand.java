...
public class HttpCommand extends Command {
    ...
    public HttpCommand() {
        this.name = "http";
        this.description = "HTTP API 测试";
        this.aliases = Arrays.asList("api", "request");
    ...
    public CommandResult execute(String args, CommandContext context) {
        ...
        for (int i = 0; i < parts.length; i++) {
            ...
            if ("-X".equals(part) || "--method".equals(part)) {
                if (i + 1 < parts.length) {
    ...
    private CommandResult showHelp() {
        String report = """
            ...
            ```
            http <URL> [-X METHOD] [-H HEADER] [-d DATA] [-t TIMEOUT]
            ```
            ...
            http -X POST https://api.example.com/users -H "Content-Type: application/json" -d '{"name":"test"}'
            http https://api.example.com/users/1 -X PUT -d '{"name":"updated"}'
            http https://api.example.com/users/1 -X DELETE
            ```
        ...
        return CommandResult.success("HTTP 帮助")
